...
class BinarySearchTree(Dictionary):
    ...
    def _inorder(self, current_node: Optional[Node] , node_ls: List[Node]) -> None:
    ...
    def retrieve(self, key):
        return self._find_item(key).value  # Return the data of the item

    def update(self, key, value):
        node = self._find_item(key)
        node.value = value

    def _replace_child(self, parent: Optional[Node] , is_left_child: bool, new_child: Optional[Node]):
    ...
    def delete(self, key):
        ...
        if (
        ...
        ):  # No left node, so replace node with right node (which may be itself null)
            self._replace_child(parent, is_left_child, current.right)

        elif (
            current.right is None
        ):  # No right node, so replace node with left node (which is not null since it's an elif)
            ...
            if successor.left is not None:  # Find the successor in the right subtree
                ...
                self._replace_child(
                    successor_parent, is_left_child=False, new_child=successor.right
                )  # Replace the successor node with its right child
            successor.left = (
                current.left
            )  # Replace left subtree of successor (which necessarily did not have a left child)
...
if __name__ == "__main__":
    bst = BinarySearchTree(
        [(14, "first"), (11, "second"), (23, "third"), (5, "fourth"), (12, "fifth")]
    )
