    ...
    def dijkstra(self, start=None):
        ...
        while len(queue) > 0:
            ...
            for v in self.get_neighbours(current):
                if v not in visited:
                    queue.append(v)
                    new_distance = distances[current] + self.get_edge_weight(
                        current, v
                    )  # Check distance
                    if new_distance < distances[v]:
                        distances[v] = new_distance
                        back[v] = (
                            current  # The new shortest distance and how to get there
                        )
...
class AdjacencyListGraph(Graph):
    ...
    def add_edge(self, vertex_from, vertex_to, weight=1):
        ...
        if not self.directed:
            ...
            )  # Add an edge back again as well
    ...
    def _recursive_depth_first(self, current, visited: set):
        ...
        for edge in self.adj_list[
            current
        ]:  # Checks all the edges from the current node
    ...
    def _get_edge_indices(self, vertex_from, vertex_to):
        ...
        if self.directed:
            ...
            if index_from == index_to:
                raise ValueError(
                    "Vertex cannot be connected to itself in undirected graph"
                )
