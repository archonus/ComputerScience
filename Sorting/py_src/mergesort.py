def top_down_merge(left_arr, right_arr):
    ...
    right_i = 0
    result = []
    while left_i < len(left_arr) and right_i < len(
        right_arr
    ):  # Compare all the elements
        if (
            left_arr[left_i] <= right_arr[right_i]
        ):  # If the one on the left_arr is <= than right_arr, add the left_arr one first
...
def bottom_up_merge(arr, start, mid, end):
    ...
    left_arr = [
        0
    ] * n1  # Initialise left_arr. It needs to be done like this to ensure that the main array is not affected
