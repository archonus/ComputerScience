...
def draw_list(display, numbers, y_offset=10):
    ...
    for i in range(len(numbers)):
        pygame.draw.rect(
            display, colourList[numbers[i] - 1], [i * 20, y_offset, 20, numbers[i] + 10]
        )  # Height of rectangles are linked to number
            ...
            while n > 1:
                for i in range(n - 1):
                    ...
                    if temp > numbers[i + 1]:
                        ...
                        clock.tick(
                            50
                        )  # Since each individual swap is being shown, it is necessary to have a high FPS
                exit = check_quit()
                if exit:
                    break  # If the user wants to stop, break the loop. Check_quit() will quit pygame internally
                n -= 1
            exit = True
...
def visualise_insertion(nums, FPS=10):
    ...
    try:
        for j in range(1, n):
            ...
            while (
                i >= 0 and nums[i] > next_num
            ):  # Move the element to the correct place
                nums[i + 1] = nums[i]
                i -= 1
            nums[i + 1] = (
                next_num  # Move next_num to the place where the while loop stopped
            )
            display.fill(black)
...
def visualise_merge_sort(numbers, FPS=5):
    ...
    while current_size < high:
        for start in range(
            0, n - 1, 2 * current_size
        ):  # Iterate from 0 to n-1, with steps of 2*current_size
            mid = (
                start + current_size - 1
            )  # The point from which left_arr stops and right_arr starts
            ...
            while left_i < len(left_arr) and right_i < len(right_arr):
                ...
                draw_left_right(
                    display, left_arr[left_i:], right_arr[right_i:], y=200
                )  # The two lists being merged, displaying only the elements that have not been merged into upper list
                pygame.display.update()
                clock.tick(FPS)
            while left_i < len(
                left_arr
            ):  # If there are elements are left over in the left side
                temp.append(left_arr[left_i])
                left_i += 1
                display.fill(black)
