...
def LoadGreyScaleImage(FileIn, Grid, Header):
  ...
  except: #Failure in the conversion to int (bad data or not enough lines in the file
...
def LoadFile(Grid, Header):
  ...
  try:
    ...
    if Header.FileType == 'A':  
      ...
      Grid = LoadGreyScaleImage(FileIn, Grid, Header) #Update grid, since header has already been assigned
      FileTypeOK = True
    else: FileTypeOK = False #Added in implicit
    FileIn.close()
    if not FileTypeOK: #The first line of the file does not have the correct metadata
      DisplayError("Unknown file type")
    else:
      DisplayImage(Grid, Header) #Once the image has been loaded, the image is displayed
  except: #This should not be a catch all
    if not FileFound:
      DisplayError("File not found")
    else: #Probably bad data - unable to convert to int
      DisplayError("Unknown error")
  return Grid, Header

def SaveFile(Grid, Header):
  FileName = input("Enter filename: ")
  FileOut = open(FileName + ".txt", 'w') #Will overwrite if file already exists
...
def GetMenuOption():
  ...
  while len(MenuOption) != 1: #The input_string must be a single char
